...
1. **Add `"venueId"`** (the listing UUID) to the create-booking (2.4) and walk-in (3.7) bodies — `venueName` alone is ambiguous when two venues share a name.
2. **OTPs currently arrive as a VOICE CALL**, not SMS (provider's SMS route pending approval). Same 6-digit code — nothing to change on your side.
3. The production database is **empty** — register a vendor through the app to create the first data. OTP limit: 3 per phone per 10 minutes.
4. `amount` is **recomputed server-side** (rate × duration + add-on prices from the listing + ₹20 fee; walk-ins: no fee). A `400 "Amount mismatch: expected ₹X"` means your calculation differs — the server's number is authoritative.
5. All error responses are `{"message": "..."}` with proper status codes (400/401/403/404/409/413/429/502).
